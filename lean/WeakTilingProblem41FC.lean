...
/-!
# Weak Tiling Problem 4.1: supports of weak tiling measures have bounded density

This file proves the `research open` theorem `WeakTiling.problem_4_1` of
[Formal Conjectures](https://github.com/google-deepmind/formal-conjectures/blob/main/FormalConjectures/Paper/WeakTiling.lean)
with the answer `True`.  The problem is Problem 4.1 of Kolountzakis, Lev and Matolcsi,
*Geometric implications of weak tiling* ([arXiv:2506.23631](https://arxiv.org/abs/2506.23631)):
if `Ω ⊂ ℝ` is a finite union of intervals and `ν` is a weak tiling measure for `Ω`, then
`supp ν` has bounded density.

The definitions `IsFiniteUnionOfIntervals`, `IsWeakTilingMeasure` and `HasBoundedDensity` are
imported unchanged from `FormalConjectures.Paper.WeakTiling`.  The final theorem is
`WeakTiling.problem_4_1_solved`.
...
theorem coverTsum_ge (hw : ∀ j, 0 < w j) (hs : Summable w) {x : ℝ} (k : ι)
  ...
  simp only [hk, and_self, if_true] at this
  exact this
...
theorem le_start_inf (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  rw [if_neg (fun h' ↦ by linarith [h'.1, min_le_left a (c j + len j), hx2])] at hgood
...
theorem end_le_inf (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  rw [if_neg (fun h' ↦ by linarith [h'.2, le_max_left b (c j), hx1])] at hgood
...
theorem exists_end_of_start_inf (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  rw [if_pos ⟨by linarith, by linarith⟩] at hgx hgx'
  ...
  have hgh : ∀ j, h j ≤ g j := by
    ...
    · have hrj : r j = 1 := by simp only [hr]; rw [if_pos hcond]
      rw [hrj, mul_one]
      by_cases hjk : j = k
      · subst hjk
        rw [if_pos rfl, if_pos hkact, if_neg hkact']
        linarith [hw j]
      · rw [if_neg hjk]
        split_ifs <;> linarith [hw j]
    · have hrj : r j = 0 := by simp only [hr]; rw [if_neg hcond]
      ...
      · have hact : c j < x ∧ x < c j + len j := ⟨by linarith, by linarith [hlen j]⟩
        ...
        rw [if_pos hact, if_neg hact', sub_zero]
        split_ifs with hjk
...
theorem exists_end_at_right_inf (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  rw [if_neg (fun h' ↦ by linarith [h'.2])] at hgx
  rw [if_pos ⟨by linarith, by linarith⟩] at hgx'
  set r : ι → ℝ := fun j ↦
...
theorem summable_coverTerm_loc (hw : ∀ j, 0 < w j) (hls : LocallySummable c w) (x : ℝ) :
  ...
  Summable.of_nonneg_of_le (fun j ↦ by split_ifs <;> linarith [hw j])
    (fun j ↦ by
      ...
      · rw [if_neg h]; split_ifs <;> linarith [hw j]) (hls x)
...
theorem le_start_half (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  obtain ⟨x, hx1, hx2, hgood⟩ := exists_good_point_half hae
    (lt_min h (by linarith [hlen j]) : c j < min a (c j + len j))
  rw [if_neg (fun h' ↦ by linarith [min_le_left a (c j + len j)])] at hgood
...
theorem exists_end_of_start_half (hae : ∀ᵐ x ∂(volume : Measure ℝ),
  ...
  rw [if_pos (by linarith)] at hgx hgx'
  set r : ι → ℝ := fun j ↦
    if (c j ≠ e ∧ |c j - e| < ε) ∨ (c j + len j ≠ e ∧ |c j + len j - e| < ε) then 1 else 0
    with hr
  have hr01 : ∀ j, 0 ≤ r j ∧ r j ≤ 1 := fun j ↦ by
...
theorem LocSum2.summable_coverTerm {ι : Type*} {c len w : ι → ℝ} (h : LocSum2 c w)
  ...
  · rw [if_neg hp]; split_ifs <;> linarith [hw p]
...
theorem right_cover (hn : 0 < n) (hwt : ∀ k, 0 < wt k)
  ...
  by_cases hin : u < x
  · rw [if_pos hin]
    rw [if_neg (right_notMem hAB hord hn hin)] at hx
    unfold coverTsum at hx
    rw [← hx]
    congr 1
    funext p
...
theorem right_semigroup (hn : 0 < n) (hwt : ∀ k, 0 < wt k)
  ...
  have hloc : LocallySummable (fun q : {p // P p} ↦ transStart A lam q.1)
    ...
    · simp only [Function.comp, if_pos hq]
      rw [if_pos]
      have := q.2
      rw [abs_le]
      constructor <;> cases abs_cases u <;> cases abs_cases R <;> linarith
    · simp only [Function.comp, if_neg hq]
...
theorem left_cover (hn : 0 < n) (hwt : ∀ k, 0 < wt k)
  ...
  by_cases hin : x < v
  · rw [if_pos hin]
    rw [if_neg (left_notMem hAB hord hn hin)] at hx
    unfold coverTsum at hx
    rw [← hx]
    congr 1
    funext p
...
theorem locSum2_of_locallyFinite (hwt : ∀ k, 0 < wt k)
  ...
  have hsg : Summable g := by
    ...
    dsimp only
    by_cases hk : |lam k| ≤ R'
    · rw [if_pos hk, Set.indicator_of_mem (show lam k ∈ Set.Icc (-R') R' from abs_le.mp hk),
        Pi.one_apply, mul_one,
        ENNReal.toReal_ofReal (hwt k).le]
    · rw [if_neg hk, Set.indicator_of_notMem (show lam k ∉ Set.Icc (-R') R' from
...
theorem coordVectorOfSemigroup_repr {ι : Type*} [Fintype ι]
  ...
  have hcoord : ∀ j, (coordVectorOfSemigroup L β g z j : ℝ) =
    ...
    intro j
    simp [coordVectorOfSemigroup, dif_pos hz, m, Nat.cast_sum, Nat.cast_mul]
  calc
    z = ∑ l : L, (m l : ℝ) * (l : ℝ) := hm
    ...
    · have hmem : x ∈ ν.support := mem_support_of_singleton_toReal_pos ν x hmasspos
      ...
      · have hnotneg : ¬ ∀ k, u k ≤ -1 := by
        ...
        have hnegCond : ¬ (u ≠ 0 ∧ ∀ k, u k ≤ -1) := by
          rintro ⟨_, hall⟩
          exact hnotneg hall
        simp only [coordinateAtomPositivePart, if_pos hpos', coordinateAtomNegativePart,
          if_neg hnegCond]
        rw [hmass]
        simp
      · have hnotpos : ¬ ∀ k, 0 ≤ u k := by
        ...
        simp only [coordinateAtomPositivePart, if_neg hnotpos, coordinateAtomNegativePart,
          if_pos hneg']
        simp
...
theorem exists_shift_solution_of_ne {α β : ℂ} (hαβ : α ≠ β) (q : ℂ[X]) :
  ...
  | succ d ih =>
    ...
    have hrc : ∀ N, d ≤ N → r.coeff N = 0 := by
      ...
      · rw [hq N hlt, Nat.choose_eq_zero_of_lt hlt, if_neg (Nat.ne_of_gt hlt)]
        simp
      · rw [Nat.choose_self, if_pos rfl, ← hc]
        field_simp
        ring
  ...
  | succ d ih =>
    ...
    have hrc : ∀ N, d ≤ N → r.coeff N = 0 := by
      ...
      · rw [hq N hlt]
        ...
        · rw [Nat.choose_eq_zero_of_lt hlt', if_neg (Nat.ne_of_gt hlt')]
          simp
        · rw [← heq, Nat.choose_self, if_pos rfl]
          ring
      · rw [Nat.choose_succ_self_right, if_neg (Nat.succ_ne_self d).symm, ← hc]
        field_simp
        push_cast
        ring
    obtain ⟨p₁, hp₁⟩ := ih r hrc
...
theorem exists_shift_solution_of_ne_deg {α β : ℂ} (hαβ : α ≠ β) :
  ...
  | succ d ih =>
    ...
    · rw [coeff_add, hp₁deg N (by omega), hp₀, coeff_C_mul, coeff_X_pow, if_neg (by omega)]
      simp
    · rw [add_comp]
      have : r = q - (C α * p₀.comp (X + 1) - C β * p₀) := hr
      rw [← hp₁] at this
  ...
  · rw [coeff_add, hrdeg μ k hk]
    ...
    · subst hμ
      have : 1 ≤ k := by
        ...
        omega
      rw [coeff_C, if_neg (by omega), zero_add]
    · simp
...
theorem top_coeff_eq_zero_of_nonneg_growth {ι : Type*} [Fintype ι] [DecidableEq ι] {d : ℕ}
  ...
  have hsplit : ∀ N, ∑ j, ∑ k ∈ range (d + 1), X j k N =
    ...
    simp only [↓reduceIte]
    rw [sum_ite_eq' (range (d + 1)) d, if_pos (mem_range.mpr (Nat.lt_succ_self d))]
  have hrest : ∀ N, 1 ≤ N →
    ...
    · simp only [hXdef, norm_mul]
      ...
      · subst hj
        ...
        simp only [hBjdef, if_pos rfl, one_mul, hTdef, hμ0, one_pow, mul_one]
...
theorem sum_le_of_unit_rep {ι : Type*} [Fintype ι] [DecidableEq ι] (b : ℕ → ℝ)
  ...
  have hterm : ∀ j, ∀ k ∈ range (d + 1),
    ...
    · rcases hkd.lt_or_eq with hlt | heq
      · rw [norm_mul, mul_assoc, if_pos h1, one_mul]
        refine mul_le_mul_of_nonneg_left ?_ (norm_nonneg _)
      ...
      · subst heq
        ...
        positivity
    · rw [norm_mul, mul_assoc, if_neg h1]
      refine mul_le_mul_of_nonneg_left ?_ (norm_nonneg _)
...
theorem pringsheim_polyExp (a : ℕ → ℝ) (ha : ∀ n, 0 ≤ a n) (S : Finset ℂ) (p : ℂ → ℂ[X])
  ...
  have hsummable : ∀ μ ∈ D, Summable fun n : ℕ ↦ ‖(p' μ).eval (n : ℂ) * (μ / R) ^ n‖ := by
    ...
    · rcases lt_or_gt_of_ne hμR with hlt | hgt
      ...
      · simp only [hp'def, if_pos hμS, hbig μ hμS hgt, eval_zero, zero_mul, norm_zero]
        exact summable_zero
    · simp only [hp'def, if_neg hμS, eval_zero, zero_mul, norm_zero]
      exact summable_zero
  ...
  simp only [hc, hp'def, if_pos hμ0] at this
  exact hμ0d this
...
theorem recPoly_natDegree (hQ : Q ≠ 0) :
  ...
  · rw [natDegree_le_iff_coeff_eq_zero]
    intro N hN
    rw [recPoly_coeff, if_neg (by omega)]
  · rw [recPoly_coeff, if_pos (by omega), Nat.sub_sub_self htm, div_self (trailing_coeff_ne_zero hQ)]
    exact one_ne_zero

theorem recPoly_monic (hQ : Q ≠ 0) : (recPoly Q).Monic := by
  have htm := natTrailingDegree_le_natDegree Q
  rw [Monic, leadingCoeff, recPoly_natDegree hQ, recPoly_coeff, if_pos (by omega),
    Nat.sub_sub_self htm, div_self (trailing_coeff_ne_zero hQ)]

theorem recPoly_coeff_zero_ne_zero (hQ : Q ≠ 0) : (recPoly Q).coeff 0 ≠ 0 := by
  rw [recPoly_coeff, if_pos (Nat.succ_pos _), Nat.sub_zero]
...
theorem recPoly_derivative_eval_one (hQ : Q ≠ 0) :
  ...
  refine sum_congr rfl fun j hj ↦ ?_
  rw [recPoly_coeff, if_pos (mem_range.mp hj)]
  ring
...
theorem recPoly_recurrence (hQ : Q ≠ 0) (N : ℂ[X]) (A : ℕ → ℂ)
  ...
  calc ∑ i ∈ range (m - t + 1), (recPoly Q).coeff i * A (k + i)
      = ∑ i ∈ range (m - t + 1), (1 / Q.coeff t) * (Q.coeff (m - i) * A (k + i)) := by
        refine sum_congr rfl fun i hi ↦ ?_
        rw [recPoly_coeff, if_pos (mem_range.mp hi)]
        ring
...
theorem endpointMonomialRow_coeff {ι : Type*} [Fintype ι]
  ...
  · rw [if_pos hs, weakTilingCoordinateRow_shift_coeff]
    by_cases hle : s ≤ e
  ...
  calc ∑ e ∈ S, coordinateAtomMass β ν (fun k ↦ (e k : ℤ))
      = ∑ e ∈ S, ‖(weakTilingCoordinateRow β ν d).coeff (coordinateFunctionExponent e)‖ := by
        ...
        have hdeg : coordinateExponentDegree (coordinateFunctionExponent e) = d := by
          ...
          exact hS e he
        rw [weakTilingCoordinateRow_coeff, if_pos hdeg]
...
theorem twoSided_bounded_recurrence (χ : ℂ[X]) (hmon : χ.Monic) (h0 : χ.coeff 0 ≠ 0)
  ...
  have hconst : p μ = C ((p μ).coeff 0) := by
    ...
    rcases k with _ | k
    · simp
    · rw [coeff_C, if_neg (Nat.succ_ne_zero k)]
...
theorem shiftedSlice_eq (D : LatticeTilingData ι) {z : ι → ℂ} (hz : ∀ k, z k ≠ 0) (m : ℤ)
  ...
  · rw [if_pos (by rw [h]; ring), if_pos h]
    ring
  · rw [if_neg (by intro h'; apply h; linarith), if_neg h, mul_zero]
...
theorem norm_latticeSlice_le (D : LatticeTilingData ι) [Nonempty ι] :
  ...
  have hnorm : ‖latticeSlice D z m‖ ≤ ∑ u ∈ F, D.M u := by
    ...
    · by_cases hM : D.M u = 0
      · simp [hdeg, hM]
      · rw [if_pos hdeg, if_pos ⟨hdeg, hM⟩, norm_mul, norm_latticeMono hz, mul_one,
...
theorem sliceXi_coeff_top (D : LatticeTilingData ι) [Nonempty ι] (z : ι → ℂ) :
  ...
  have hsum : ∑ j, ((if sliceTop D = sliceTop D - ∑ k, D.p j k then
    ...
    refine sum_congr rfl fun j _ ↦ ?_
    rw [if_neg (hq j), sub_zero]
    by_cases h : D.p j = 0
    · rw [if_pos ((hp j).mpr h), if_pos h, hmono0 j h, Nat.cast_one]
    · rw [if_neg (fun h' ↦ h ((hp j).mp h')), if_neg h]
...
theorem sum_norm_fiber_le (D : LatticeTilingData ι) [Nonempty ι] :
  ...
  · by_cases h : D.M u = 0
    · simp [hdeg, h]
    · rw [if_pos hdeg, if_pos ⟨hdeg, h⟩, Complex.norm_real, Real.norm_of_nonneg (D.nonneg u)]
  · simp [hdeg]
...
theorem eval_pronyPoly (a : ℕ → ℂ) (k : ℕ) (y : ℂ) :
  ...
  simp only [pronyMat, of_apply, Fin.val_last, lt_irrefl, if_false]
  ring
...
theorem pronyPoly_coeff_top (a : ℕ → ℂ) (k : ℕ) :
  ...
  · rw [coeff_C_mul_X_pow, Fin.val_last, if_pos rfl, ← two_mul, pow_mul, neg_one_sq, one_pow,
      one_mul, pronyMinor]
    congr 1
    ext i j
...
theorem hankel_det_ne_zero (A : Finset ℂ) (c : ℂ → ℂ) (hc : ∀ μ ∈ A, c μ ≠ 0) :
  ...
  have hfac : hankel (expSeq A c) A.card =
    ...
    simp only [hankel, expSeq, of_apply, mul_apply, transpose_apply, vandermonde, diagonal_apply,
      ite_mul, zero_mul, sum_ite_eq, mem_univ, if_true]
...
theorem pronyPoly_eval_eq_zero (A : Finset ℂ) (c : ℂ → ℂ) {μ : ℂ} (hμ : μ ∈ A) :
  ...
  have hfac : pronyMat (expSeq A c) A.card μ =
    ...
    · simp only [if_pos hi, expSeq]
      rw [← sum_coe_sort A]
      refine sum_congr rfl fun ν _ ↦ ?_
      rw [pow_add]
      ring
    · simp only [if_neg hi, ite_mul, one_mul, zero_mul]
      rw [sum_coe_sort A (fun ν ↦ if ν = μ then ν ^ (j : ℕ) else 0), sum_ite_eq' A μ, if_pos hμ]
  rw [hfac]
...
theorem phase_affine [Nonempty ι] [DecidableEq ι] (hN : IsOpen N) (B : ActiveBranches D N)
  ...
  have hW_smooth : ∀ r, ∀ t ∈ Ioo (-δ) δ, ContDiffAt ℝ 2 (W r) t := by
    ...
    simp only [hW]
    rw [dif_pos (isUnit_iff_ne_zero.mpr (hprod_ne t' ht')), extractionWeight_eq]
...
theorem zero_mem_genericSet [Nonempty ι] : (0 : ι → ℝ) ∈ genericSet D := by
  ...
  have hp : ∀ j, ¬ (0 = sliceTop D - ∑ k, D.p j k) := by
    ...
    omega
  simp only [hp, if_false, zero_sub, sum_neg_distrib, neg_ne_zero]
  rw [sum_boole]
...
theorem exists_separating_direction {α : Type*} (S : Finset α) (ξ : α → σ → ℝ)
  ...
  rw [if_neg hne, Multiset.mem_toFinset, mem_roots (hpoly_ne _ hv), IsRoot, heval]
  simp only at hab
...
Conjectures with the answer hole filled by `True`. -/
